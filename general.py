...
# Verify if a path is a file path
...
def extractLocationPath(link, initialLocation=''):
	...
	if (result.netloc == ''):
		return urlunparse(ParseResult(location.scheme, location.netloc,result.path,'','',''))
	else:
		return urlunparse(ParseResult(result.scheme, result.netloc,result.path,'','',''))
